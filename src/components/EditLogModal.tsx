  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!weight || !difficulty) {
      Swal.fire({
        ...
        text: 'El peso y la dificultad son obligatorios.',
        icon: 'warning',
        background: '#1e293b',
        color: '#e2e8f0',
      });
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="animate-fadeIn w-full max-w-sm rounded-2xl border border-slate-700 bg-slate-900 p-6"
      >
        ...
        <form onSubmit={handleSave} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-slate-700 bg-slate-800 px-4 py-2.5 text-center text-lg font-bold"
            />
          </div>
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {(['Fácil', 'Medio', 'Difícil'] as Difficulty[]).map((d) => (
                <button
                  ...
                  className={`rounded-lg border py-2.5 text-sm font-semibold transition-all ${difficulty === d ? `${difficultyStyles[d]} scale-105` : 'border-slate-700 hover:bg-slate-800/50'}`}
                >
                  {d}
                </button>
              ))}
          ...
          <button
            type="submit"
            disabled={isSaving}
            className="w-full rounded-lg bg-emerald-500 py-3 font-bold text-slate-950 transition hover:bg-emerald-400 disabled:opacity-50"
          >
