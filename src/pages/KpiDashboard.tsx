    ...
    <div style={{ width: '100%', height: 300 }}>
      <ResponsiveContainer>
        <LineChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
          ...
          <Tooltip 
            contentStyle={{ backgroundColor: '#1f2937', borderColor: '#374151', color: '#f1f5f9' }}
        ...
        history.length === 0 ? (
          ...
          <div className="space-y-8">
            ...
            <div>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-2 max-h-72 overflow-y-auto">
                  <div className="space-y-1">
                      {history.map(log => (
                          <div key={log.id} className="flex items-center bg-slate-800/40 p-3 rounded-lg hover:bg-slate-800/60">
                              <span className="font-semibold text-slate-400 text-sm flex-1">{new Date(log.created_at).toLocaleDateString()}</span>
                              <span className="font-black text-lg text-slate-200 flex-1 text-center">{log.weight_kg} kg</span>
                              <div className="flex-1 flex justify-center">
                                  {log.difficulty && (
                                      <span className={`font-semibold px-2.5 py-1 rounded-full text-xs ${difficultyStyles[log.difficulty]}`}>
                                          {log.difficulty}
                                      </span>
                                  )}
                              </div>
                              <div className="flex-1 flex justify-end">
                                  <button onClick={() => setEditingLog(log)} className="text-slate-300 bg-slate-800/30 hover:bg-slate-700 px-3 py-1 rounded-md text-sm">
                                      Editar
                                  </button>
                              </div>
                          </div>
